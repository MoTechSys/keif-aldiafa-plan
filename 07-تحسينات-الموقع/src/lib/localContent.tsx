...
export function getLocalContent(service: string, cityKey: string): {
  ...
  // إشارة محلية فريدة: أول حيّين من أحياء المدينة — يجعل كل وصف مدينة متمايزاً
  // فعلياً (يقلّل خطر duplicate/thin content لنفس الخدمة عبر المدن).
  const localHook = c.districts.slice(0, 2).join(" و");
  const metaDescription = isMunasabat
    ? `خدمة ضيافة مناسبات في ${c.ar} (${localHook} وكل الأحياء): تجهيز أعراس ومؤتمرات وفعاليات بطاقم فاخر ومعدات كاملة. عرض سعر مجاني — واتساب ${WA_DISPLAY}.`
    : isQahwajiin
    ? `قهوجيين ومباشرين وقهوجي في ${c.ar} (${localHook} وكل الأحياء) بزيّ رسمي لخدمة كبار الضيوف في الفعاليات والمؤتمرات وبروتوكول VIP. عرض سعر مجاني — واتساب ${WA_DISPLAY}.`
    : `صبابين قهوة سعوديين في ${c.ar} (${localHook} وكل الأحياء) — صبّابون وصبابات بزيّ تراثي لتقديم القهوة العربية في الأعراس والمناسبات. عرض سعر مجاني عبر واتساب ${WA_DISPLAY}.`;
